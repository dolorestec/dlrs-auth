...
async def test_get_by_email_found(repo):
    ...
    mock_conn.fetchrow.return_value = {
        'id': 1, 'email': 'test@example.com', 'hashed_password': 'hash',
        'is_active': True, 'created_at': '2023-01-01', 'updated_at': '2023-01-01'
    }
...
async def test_create_user(repo):
    mock_conn = repo._pool.acquire.return_value.__aenter__.return_value
    mock_conn.fetchrow.return_value = {
        'id': 1, 'email': 'new@example.com', 'hashed_password': 'hash',
        'is_active': True, 'created_at': '2023-01-01', 'updated_at': '2023-01-01'
    }
